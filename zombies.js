...
class Player {
  ...
  takeItem(item) {
    if (this._pack.length < 3) {
      console.log(Player.name + ' has ' + item);
      this._pack.push(item);
      return true;
    }
    else {
      console.log('pack full, item could not be stored');
      return false;
    }
  }
  discardItem(item) {
  ...
  checkPack() {
    console.log('items ' + this.getPack);
  }
  equip(itemToEquip) {
    if (itemToEquip instanceof Weapon === true && this._pack.includes(itemToEquip)) {
      if (this._equipped !== false) {
        ...
        this.equipped = this._pack.splice(inPack, 1);
  ...
  eat(itemToEat) {
    if (itemToEat instanceof Food === true && this._pack.includes(itemToEat)) {
      ...
      if (this._maxHealth < this.health + itemToEat.energy) {
        this.health = this._maxHealth;
        // this.health = itemToEat;
      }
      // else {
      //   this._pack.splice(this._pack.indexOf(itemToEat),1);
  ...
  equippedWith() {
    if (this.equipped) {
      console.log(Player.name + ' equipped with ' + Weapon.name);
      return this.equipped.name;
    }
    else {
      console.log(Player.name + ' not equipped');
      return false;
    }
  }
